...
export const TradingOrderForm = ({ pairIndex }: IOrderForm) => {
  ...
  const [marginAssets, setMarginAssets] = useState({
    marginAssetDrop: getNetwork(chain === undefined ? 0 : chain.id).marginAssets
  });

  const [currentMargin, setCurrentMargin] = useState({
    marginAssetDrop: getNetwork(chain === undefined ? 0 : chain.id).marginAssets[0]
  });
    ...
    <Container>
      ...
      <FormContainer>
        ...
        <FormArea>
          <TigrisInput
            label="Price"
            value={orderType === 'Market' ? getOpenPrice().replace('NaN', '') : openPrice.replace('NaN', '')}
          ...
          <TigrisInput
            label="Stop Loss"
            placeholder={'-'}
            value={stopLossPercent === '0' ? '' : isSlFixed ? stopLossPrice : getStopLossPrice().replace('NaN', '-')}
        ...
        <ApproveButton onClick={() => routeTrade()} isOnline={getButtonOnline()}>
          {getButtonText()}
        </ApproveButton>
        <Alert>
  ...
  async function getTokenBalance() {
    ...
    const tokenContract = new ethers.Contract(
      ...
      currentNetwork.abis.erc20,
      provider
    );
    const balance = ((await tokenContract.balanceOf(address)) / 10 ** currentMargin.marginAssetDrop.decimals).toFixed(
      2
    );
  ...
  */
  ...
  function getButtonOnline() {
    ...
    const hasOnline = s === 'Approve' || s === 'Proxy' || s === 'Ready';
    return hasOnline;
  }
      ...
      s === 'Approve'
        ...
        ? (isLong ? 'LONG $' : 'SHORT $') +
          Math.round(parseFloat(margin) * parseFloat(leverage))
            ...
            .replace(/\B(?=(\d{3})+(?!\d))/g, ',') +
          ' ' +
          currentNetwork.assets[pairIndex].name
        : s === 'NotConnected'
        ? 'CONNECT WALLET'
        : s === 'Balance'
        ? 'NOT ENOUGH BALANCE'
  ...
  async function approveProxy() {
    ...
    const tx = tradingContract.approveProxy(await getShellAddress(), now + 86400, {
  ...
  async function getTokenApproval() {
    ...
    const currentNetwork = getNetwork(chain === undefined ? 0 : chain.id);
    if (
      currentMarginRef.current !== null &&
      currentMarginRef.current.marginAssetDrop.address === currentNetwork.addresses.tigusd
    ) {
    ...
    const provider = new ethers.providers.Web3Provider(ethereum);
    const tokenContract = new ethers.Contract(
      currentMarginRef.current.marginAssetDrop.address,
      currentNetwork.abis.erc20,
      provider
    );
